...
**เพิ่มบัญชีใหม่ (ทำจากในเว็บ ไม่ต้องแตะ SQL)** — เมนู "ผู้ใช้และผู้ขาย"
1. Supabase → Authentication → Users → Add user (อีเมล + รหัส + ติ๊ก **Auto Confirm**)
2. กลับมาที่เว็บ การ์ด **"บัญชีที่ยังไม่ได้ผูก"** จะขึ้นบัญชีนั้นให้ → เลือก **ประเภทบัญชี**
   - **ซัพพลายเออร์** → เลือกบริษัท → กดผูก
   - **ฝ่ายจัดซื้อ** → ใส่ตำแหน่ง → กดให้สิทธิ (ได้สิทธิดูแลระบบเต็ม เห็นงบและราคาคาดหวังทุกงาน)
3. บริษัทใหม่เพิ่มได้จากการ์ด **"เพิ่มบริษัทผู้ขาย"** ในหน้าเดียวกัน

ถอนสิทธิได้จากปุ่มท้ายแถว ถอนบัญชีตัวเองไม่ได้ และบัญชีที่เคยยื่นราคาแล้วถอนไม่ได้ (ประวัติจะขาด)

**ตัวเลขที่ผู้ขายไม่เห็นเลย** — `งบประมาณ` และ `ราคาคาดหวัง` อยู่ในตาราง `tender_internal`
ซึ่งมีนโยบายให้เฉพาะฝ่ายจัดซื้อ ไม่ใช่แค่ซ่อนในหน้าจอ (RLS กันได้ทีละแถว จึงต้องแยกตาราง ไม่ใช่แยกคอลัมน์)
...
│   └── 14_open_period.sql       ช่วงเวลาประมูล: กำหนดเวลาเปิดรับราคาเองได้
├── scripts/
