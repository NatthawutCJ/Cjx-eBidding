-- ============================================================================
-- ช่วงเวลาประมูล: กำหนด "เปิดรับราคา" ได้เอง ไม่ใช่เปิดทันทีอย่างเดียว
--
-- ตาราง tenders มีคอลัมน์ opens_at อยู่แล้ว (default now()) และ tender_status()
-- คืนค่า 'scheduled' ให้เองเมื่อยังไม่ถึงเวลา ส่วน submit_bid() ก็กันไม่ให้ยื่นก่อนเปิดอยู่แล้ว
-- ที่ขาดคือ create_tender() ไม่เคยรับค่า opens_at จากหน้าเว็บ ทุกงานจึงเปิดทันทีเสมอ
--
-- ไฟล์นี้แทนที่ create_tender() ตัวเดิม ไม่แตะข้อมูลที่มีอยู่ รันซ้ำได้
--   cat ~/Desktop/supplier-bidding/app/supabase/14_open_period.sql | pbcopy
-- ============================================================================
...
-- ตรวจผล: ควรได้ true ทั้งสองคอลัมน์
select
  pg_get_functiondef(p.oid) like '%opens_at%'                as "รับเวลาเปิดรับแล้ว",
  pg_get_functiondef(p.oid) like '%ต้องมาก่อนเวลาปิดรับ%'      as "มีการตรวจช่วงเวลา"
