...
const SAMPLE = {
  ...
  remark: 'ราคาที่เสนอต้องรวมค่าขนส่งถึง DC และภาษีมูลค่าเพิ่มแล้ว\nชำระเงิน 30 วันหลังรับของครบ',
  items: [
  ...
  docs: ['ใบเสนอราคาลงนาม (PDF)'],
  specNote: 'ถ้ามี Spec สินค้า / แคตตาล็อก / รายละเอียดทางเทคนิค โปรดแนบมาด้วย',
}
...
const invite = d => ({
  ...
  html: shell(`${d.company} ได้รับเชิญเสนอราคา ปิดรับ ${d.closesAt}`, `
    ...
    ${rows([
      ...
      ['ช่วงเวลาประมูล', `${esc(d.opensAt)} — <b>${esc(d.closesAt)}</b> (ตามเวลาระบบ)`],
      ['เปิดซอง', `${esc(d.unsealAt)} พร้อมกันทุกราย`],
      ['ยืนราคา', esc(d.validity)],
      ['ส่งมอบ', esc(d.delivery)],
      ...(d.remark ? [['หมายเหตุจากผู้ซื้อ', esc(d.remark).replace(/\n/g, '<br>')]] : []),
    ])}
    ...
    ${checklist(d.docs)}
    <div style="font-family:${FONT};font-size:13px;color:${DIM};padding-top:8px">
      ${esc(d.specNote)}</div>
